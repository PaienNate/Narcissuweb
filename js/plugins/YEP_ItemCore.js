 ...
 * ============================================================================
 *
 * 物品核心插件改变了游戏的很多内容，从物品菜单到个人包裹，更重要的是这个插
 * 件为希望设置更多物品选项的人搭建了桥梁。
 * 
 * 你们好，我知道私人包裹有最大数量限制。这个最大数量限制只是为了限制玩家购
 * 买太多的物品，导致垃圾填满了仓库。你可以用其他方式来获得物品，例如事件，
 * 战斗掉落等。
 * 
 * 为什么呢？因为开发者不可能考虑所以情况。大多数下，开发者不能知道玩家的仓
 * 库容量。所以，为了减轻这种情况，除非有事件或者战斗掉落，否则玩家只能被限
 * 制数量。
 * 
 * 又为什么呢？因为关键物品。在游戏中，通过事件或者战斗给与关键物品是游戏重
 * 要的换届。如果开发者不知道玩家库存，并且提供的关键物品无法进入仓库，玩家
 * 将会被锁住游戏进程不能游戏。所以优先考虑玩家，这个系统被打开。
 * 
 ...
 *
 * 1. Independent Items
 * 如果你想要有物品的最大数量限制，那么这些物品会有独立的统计等。独立的。独
 * 立物品可以被提升，改变，修改等同时保留物品类型。没有最大数量限制的物品，
 * 他们将会以默认的物品管理方式运行。
 *
 * 2. New Scene_Item
 * 物品界面看起来略微不同。有了分层，物品列表不在是2列，而是一列。并且添加
 * 了更多的界面，例如物品重要程度，物品信息，物品动作等。如果你不想使用这个
 * 修改，你可以用参数关闭它。
 *
 * 3. Random Variance
 * 不是从商店买来的物品可以一定程度上获得随机属性。数值可以在一个范围内浮动
 * ，如果你不希望有浮动，你可以使用标签设置为0.如果你希望所有物品都没有浮动
 * ，你可以设置参数为0.
 *
 ...
 *
 * 有个小提示需要注意，如果你开启了固定值，玩家重新通过游戏标题界面再次进入
 * 游戏或者关闭程序重新启动，随机值将会开启。插件命令只是一个暂时的关闭.
 *
 * ============================================================================
 * Eventing Changes
 * ============================================================================
 ...
 *   条件及分支：当检查你的仓库里是否存在物品时，独立物品的+1属性不会造成影响
 * ，可以认为物品存在
 *
 * Actor Has X Equip:
 *   装备：当检查你的装备里是否存在物品时，独立装备的+1属性不会造成影响，可以
 * 认为物装备存在
 *
 * Change Equipment:
 *   装备改变：如果装备标签是独立物品，这个游戏会检查基础装备来适配，如果没找
 * 找到，则会检查个人仓库；如果依旧没有，则会创建一个类别。
 *
 * ============================================================================
 * Item Name System
 * ============================================================================
 ...
 *
 * 前缀，名字，后缀，增量可以用插件来调整。如下便是一个例子：前缀Fiery，名
 * 字Sword，后缀of Might，增量5.
 *
