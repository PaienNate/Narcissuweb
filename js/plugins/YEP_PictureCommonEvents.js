 ...
 * ============================================================================
 *
 * 图片不仅仅可以作为图像存在你的游戏，还可以实现很多功能。他们可以用来做界
 * 面元素。可可目前他们还不能在鼠标点击时作为公共事件执行。使用这个插件，图
 * 片可以绑定公共事件，当鼠标点击或者触摸时执行
 *
 * ============================================================================
 * Instructions
 * ============================================================================
 *
 * 在插件参数里，你可以绑定图片的事件ID，总共图片上限100张。只要图片有值，他
 * 就会用于事件。这里有4种触发情况:
 *
 * ---
 *
 * - CLICK - 点击：当图片被单击时触发。事件需要玩家点击鼠标并释放后才触发
 ...
 *
 * - REPEAT - 重复：如果鼠标按下并不放松，在释放之前，每6帧就会执行一次
 ...
 *
 * - HOLD - 按住：如果鼠标按下并且不放松。在释放之前，每帧都会执行一次
 * If the mouse is clicked and then held (or if the touch screen is pushed and
 * held), as long as the mouse is above the picture area and the common event
