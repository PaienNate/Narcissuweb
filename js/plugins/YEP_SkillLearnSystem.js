 ...
 * ============================================================================
 *
 * 此插件可以让游戏中的角色在技能菜单中学习技能。学习技能的代价可以是金钱、
 * 物品或所谓的“职业点数”（Job Point）。它能够提供除了升级以外的获得技能的
 * 可选方式。
 *
 * Fox的JP（职业点数）等级——这个插件向Yanfly的JP系统中添加了等级概念，就像
 * 《最终幻想战略版》中那样。根据你当前职业的“职业等级”（Job Level），你可
 * 以在SkillLearnSystem中获取相应的技能，也可以在ClassChangeCore中更换相应的
 * 业。而职业等级只是通过增加JP来提高的。
 * 
 ...
 *   <Learn Skill: x, x, x>
 *   <Learn Skill: x to y>
 *   让此职业能够在菜单中学习技能x。如果是第三种情形x to y，它允许职业学习从x
 *   到y的所有技能，其中x和y是技能的ID。
 *
 * 技能注释:
 *   <Learn Cost: x Gold>
 ...
 *   </Learn Cost>
 *   设置学习此技能所要花费的物品、武器和防具，其中x是物品的ID，y是所需物品的
 *   数量。如果你打算用物品的名称作为变量，那么把item name换为物品在数据库中的
 *   名称。但如果有多个物品的名称相同，那么将会使用ID最大的东西（按照“物品＞
 *   武器＞防具”的顺序）作为费用。
 *   *注意：如果你在使用YEP_ItemCore.js插件及“独立物品”（Independent Items）
 *   ，那么学习费用将不包括那些独立物品。
 *
 ...
 *   角色必须已经掌握其他技能x，此技能才会出现（在列表上）。如果是第三种情形x 
 *   to y，角色必须已经掌握从x到y的所有技能，其中x和/或y是技能的ID。
 *
 *   <Learn Require Switch: x>
 *   <Learn Require Switch: x, x, x>
 *   <Learn Require Switch: x to y>
 *   开关x必须已被打开，此技能才会出现。如果是第三种情形x to y，从x到y的所有开
 *   关必须已被打开，技能才会出现，其中x和/或y是开关的ID。
 *
 * ============================================================================
 * “疯狂模式”——自定义要求和花费
 * ============================================================================
 *
 * 如果你只有很少的JavaScript使用经验，并且希望更好地定做技能学习的方法，那
 * 么你可以用下列注释：
 *
 * 技能的注释：
 *
 ...
 *    value = false;
 *   </Learn Show Eval>
 *   如果想用一串代码来隐藏或显示此技能，你可以使用这些注释。若返回value的值为
 *   true，那么即使其它要求不被满足，技能也会出现（在列表上）。而若返回value的
 *   值为false，那么即使其它要求被满足，技能也不会出现。
 *
 *   <Learn Require Eval>
 *    value = true;
 *    value = false;
 *   </Learn Require Eval>
 *   给希望通过代码设置自己特定的技能学习要求的人使用。只有返回value的值为true
 *   ，技能才会显示为可学的。请避免使用可以停止后续代码工作的命令。
 *   code.
 *
 *   <Learn Cost Eval>
 *    code
 ...
 * ============================================================================
 *
 * 可以从事件编辑器（Event Editor）中使用下列插件命令，来确定你是否想让“学
 * 习技能”命令出现在技能界面以使之可行。
 *
 * Plugin Command:
