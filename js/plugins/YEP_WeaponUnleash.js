 ...
 * ============================================================================
 *
 * 这个插件可以让你的游戏代替攻击和防御命令，或者放入技能的选项。这个插件参
 * 考了Golden Sun的插件
 *
 * 当替代攻击和防御命令，可以改变命令的名字。这些替换不仅仅在装备，也可以是
 * 角色，职业，敌人，状态等等
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 *
 * 这里有一些标签，你可以使用它们来替代默认的攻击命令，提供武器发动效果。注
 * 意：如果你使用了标签写了技能名字，并且你有多个重复名字的技能，那么将会优
 * 先自行ID最高的技能。
 *
 ...
 *   <Replace Attack: x>
 *   <Replace Attack: name>
 *   这个将会替代攻击技能。如果你的攻击命令有多个替代，会按照下面的顺序，先进
 *   先生效:
 *
 ...
 *   <Replace Guard: x>
 *   <Replace Guard: name>
 *   这个将会替代防御技能。如果你的防御命令有多个替代，会按照下面的顺序，先进
 *   先生效
 *
 *   States - Highest to Lowest Priority  状态 从高到低
 *   Equipment - In order of equip list   装备 装备列表顺序
 ...
 *   这会造成攻击技能有x%的概率使用技能y。这仅仅在攻击命令生效。你可以插入多个
 *   副本来生成更多的武器发动技能。如果一个战斗者有多个武器发动技能，会按照下
 *   面顺序随机计算
 *
 *   States - Highest to Lowest Priority  状态 从高到低
 *   Equipment - In order of equip list   装备 装备列表顺序
 ...
 *   Actor Aspect  角色
 *
 *   如果武器发动效果在列表中较早通过，并且后续列表还有武器发动效果，那么后续
 *   发动效果会被优先重写。
 *
 *   --- Guard Unleash ---
 *
 ...
 *   这会造成防御技能有x%的概率使用技能y。这仅仅在防御命令生效。你可以插入多个
 *   副本来生成更多的武器发动技能。如果一个战斗者有多个武器发动技能，会按照下
 *   面顺序随机计算:
 *
 ...
 *   Actor Aspect
 *
 *   如果防御发动效果在列表中较早通过，并且后续列表还有防御发动效果，那么后续
 *   发动效果会被优先重写。
 *
 ...
 *   <Weapon Unleash name: -y%>
 *   改变武器发动效果为技能x的概率增加或者减少y%。如果你使用了名字标签并且拥有
 *   多个重复的名字，优先执行ID最高的
 *
 *   <Guard Unleash: +x%>
 ...
 *   <Guard Unleash name: -y%>
 *   改变防御发动效果为技能x的概率增加或者减少y%。如果你使用了名字标签并且拥有
 *   多个重复的名字，优先执行ID最高的
 *
 * Skill Notetags:
 * 
 ...
 *   <Attack Text: x>
 *   <Guard Text: x>
 *   如果你替代了攻击或者防御命令，你可以改变显示的命令名字。如果你使用
 *   <Command Text: x>的标签，这将应用于攻击和防御。
 *
 * ============================================================================
