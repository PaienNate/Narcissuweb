 ...
 *
 * Cooldown (Standard)
 * 标准冷却。当被使用时，技能需要等x回合才可以使用。这里可以利用某些东西
 * 让冷却降低。最简单的就是等待，每回合都会降低。技能或者类似的也可以降低
 * 冷却。除此之外，你还可以结束战斗，战斗结束后冷却都会到一个定值。第三种
 * 方法就是在地图上走动，走动步数可以降低冷却
 *
 * Warmups
 * 就大多数而言，热身和冷却一样，时间结束才可以使用技能，区别是，热身仅仅
 * 发生在战斗一开始。如果一个技能有热身时间，他会进入战斗中触发，战斗结束
 * 后小时。热身不会叠在任何冷却之上，当技能同时拥有冷却和热身时，会同步更新
 *
 * Linked Cooldowns
 * 关联冷却是某个技能造成另一个技能冷却时。所有的冷却都看做关联冷却。冷却
 * 将会优先考虑技能冷却和全局冷却
 *
 * Skill Type Cooldowns
 * 当技能类型冷却时，所有匹配的技能类型都在冷却中。另外的技能将会是标准冷
 * 却。技能冷却优先考虑全局冷却
 *
 * 当冷却应用于已存在冷却的技能，冷却将会改变最大值。这意味着，当一个技能
 * 有3回合冷却时，技能类型冷却设置为1回合，则3回合会保留。如果技能有3回合
 * 冷却，技能类型冷却设置为5回合，冷却时间变更为5回合
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 ...
 * 技能注释:
 *   <Cooldown: x>
 *   设计技能冷却回合数x。这只会影响技能自身。这个值会优先考虑技能类型冷却
 *   和全局冷却
 *
 *   <Warmup: x>
 *   设计技能热身回合数x。这只会影响技能自身。这个值会优先考虑技能类型冷却
 *   和全局冷却
 *
 ...
 *   使用技能后，并且支付技能消耗以后，技能x将会关联冷却y回合。这个值优先
 *   考虑技能类型冷却和全局冷却
 *
 *   <SType x Cooldown: y>
 *   使用技能后，并且支付技能消耗以后，所有技能类型x将会关联冷却y回合。这
 *   个值优先考虑全局冷却
 *
 *   <Global Cooldown: x>
 *   使用技能后，所有技能将会设置冷却x回合。这个值优先于独立冷却和技能类
 *   型冷却
 *
 *   <Bypass Cooldown>
 ...
 *   <Skill name Cooldown: +y>
 *   <Skill name Cooldown: -y>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，这个技
 *   能x冷却将会改变y。这将在所有计算完成后应用一个平均冷却。
 *
 *   <SType x Cooldown: +y>
 *   <SType x Cooldown: -y>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，这个技
 *   能类型x冷却将会改变y。这将在所有计算完成后应用一个平均冷却。
 *
 *   <Global Cooldown: +x>
 *   <Global Cooldown: -x>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，全部技
 *   能冷却将会改变x。这将在所有计算完成后应用一个平均冷却。
 *
 *   <Skill x Warmup: +y>
 *   <Skill x Warmup: -y>
 *   <Skill name Warmup: +y>
 *   <Skill name Warmup: -y>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，战斗开
 *   始时，技能x热身时间改变y。这将在所有计算完成后应用一个平均热身时间。
 *
 *   <SType x Warmup: +y>
 *   <SType x Warmup: -y>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，战斗开
 *   始时，技能类型x热身时间改变y。这将在所有计算完成后应用一个平均热身时间。
 *
 *   <Global Warmup: +x>
 *   <Global Warmup: -x>
 *   如果玩家是特点玩家，职业，或者装备特定物品时，或者存在特点状态，战斗开
 *   始时，全部技能热身时间改变x。这将在所有计算完成后应用一个平均热身时间。
 *
 * ============================================================================
